...
import { loadAdminPendingTools } from '@/lib/db/queries';
...
function StatusBadge({ status }: { status: string }) {
  const s =
    status === 'ai_drafted'
      ? { label: 'AI 草稿', color: C.amber, bg: C.amberBg }
      : { label: status, color: C.orange, bg: C.orangeBg };
  return (
    <span
      style={{
...
export default async function AdminToolsPage({ searchParams }: Props) {
  ...
  const { items, total } = await loadAdminPendingTools(PAGE_SIZE, offset);
                ...
                {items.length === 0 ? (
                  ...
                  items.map((item, i) => (
                    ...
                    >
                      ...
                      <td style={{ padding: '10px 14px', color: C.inkSoft }}>
                        {item.sourceName}
                      </td>
                      <td
                        style={{
