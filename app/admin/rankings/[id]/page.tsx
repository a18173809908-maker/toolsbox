...
import { loadRankingDraftById } from '@/lib/db/queries';
...
export default async function Page({ params }: Props) {
  ...
  const item = await loadRankingDraftById(id);
  if (!item) notFound();
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Link href="/admin/rankings" style={{ color: C.inkSoft, textDecoration: 'none', fontSize: 13 }}>← 排行榜草稿</Link>
            <h1 style={{ margin: 0, color: C.ink, fontSize: 20 }}>审核排行榜草稿</h1>
          </div>
          <AdminLogoutButton />
        </div>
        ...
        <DraftReviewActions id={id} apiBase="/api/admin/rankings" listPath="/admin/rankings" publishLabel="发布排行榜草稿" />
      </div>
    </div>
  );
}
