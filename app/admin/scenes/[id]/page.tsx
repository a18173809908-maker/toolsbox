...
import { loadSceneDraftById } from '@/lib/db/queries';
...
export default async function Page({ params }: Props) {
  ...
  const item = await loadSceneDraftById(id);
  if (!item) notFound();
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Link href="/admin/scenes" style={{ color: C.inkSoft, textDecoration: 'none', fontSize: 13 }}>← 场景草稿</Link>
            <h1 style={{ margin: 0, color: C.ink, fontSize: 20 }}>审核场景草稿</h1>
          </div>
          <AdminLogoutButton />
        </div>
        ...
        <DraftReviewActions id={id} apiBase="/api/admin/scenes" listPath="/admin/scenes" publishLabel="发布场景草稿" />
      </div>
    </div>
  );
}
