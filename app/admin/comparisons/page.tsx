...
import { loadAdminDraftComparisons } from '@/lib/db/queries';
...
export default async function AdminComparisonsPage({ searchParams }: Props) {
  const { page: pageStr } = await searchParams;
  const page = Math.max(1, parseInt(pageStr ?? '1', 10) || 1);
  const offset = (page - 1) * PAGE_SIZE;

  const { items, total } = await loadAdminDraftComparisons(PAGE_SIZE, offset);
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        {totalPages > 1 && (
          ...
          >
            {page > 1 && (
              <Link
                href={`/admin/comparisons?page=${page - 1}`}
                style={{
                  padding: '6px 14px',
                  border: `1px solid ${C.rule}`,
                  borderRadius: 6,
