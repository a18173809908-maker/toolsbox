...
import { loadToolFieldDraftById } from '@/lib/db/queries';
...
export default async function Page({ params }: Props) {
  ...
  const item = await loadToolFieldDraftById(id);
  if (!item) notFound();
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <Link href="/admin/tool-fields" style={{ color: C.inkSoft, textDecoration: 'none', fontSize: 13 }}>← 工具字段草稿</Link>
            <h1 style={{ margin: 0, color: C.ink, fontSize: 20 }}>审核工具字段草稿</h1>
          </div>
          <AdminLogoutButton />
        </div>
        ...
        <DraftReviewActions id={id} apiBase="/api/admin/tool-fields" listPath="/admin/tool-fields" publishLabel="发布字段草稿" />
      </div>
    </div>
  );
}
