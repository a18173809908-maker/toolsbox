...
> **执行约束**：当前所有需要人工参与的内容暂不做。真人实测 Lab、人工审校、人工分发、厂商触达均后置；Sprint 3 只推进可自动化、doc-based、官方来源支撑的工程和内容闭环。
...
- Lab 视频实测报告仅预留能力，当前不作为交付目标。
- 至少 1 个视频工具替代品专题上线。
- 视频类工具池补齐到能支撑 10 篇内容。
- 每篇内容保持 doc-based 标注；涉及画质、速度、稳定性的强判断必须明确写“待实测”或不下结论。
...
**扩容（2026-05-09）**：✅ 已完成。新增发布 `kling-vs-jimeng`、`kling-vs-hailuo`、`runway-vs-luma`、`sora-vs-runway`、`pika-vs-luma`、`jimeng-vs-hailuo`；视频类已发布对比页扩展到 10 篇，新增 6 篇均 `reviewed_by='admin'`，正文约 4973-5296 字符，自动审计风险为 0。
...
**状态（2026-05-09）**：暂缓。当前所有需要人工参与的内容暂不做，本任务只作为未来人工实测池保留。

未来推荐选题：`kling-vs-runway`。
...
4. K5：补 Runway 替代品专题。
5. K4：暂缓，等用户确认可以投入人工实测后再做。

K4 不阻塞任何当前任务。视频 Lab 成本高且需要真人参与，当前先用 doc-based 页面、替代品专题和内链结构跑通自动化闭环。
