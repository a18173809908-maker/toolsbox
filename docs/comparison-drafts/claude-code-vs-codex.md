...
如果你主要在本地开发环境里工作，希望 AI 直接理解当前目录、运行命令、改文件，并且愿意围绕 `CLAUDE.md`、权限、子代理和 MCP 做较细的工作流配置，优先选 **Claude Code**。

如果你已经深度使用 ChatGPT / OpenAI 生态，希望把任务交给云端并行跑，让它在隔离环境里读代码、改代码、跑测试、起草 PR，优先选 **OpenAI Codex**。

一句话：**Claude Code 更像一个贴着本地开发环境走的搭档；Codex 更像一个可以被派发任务的云端工程代理。**
...
|---|---|---|
| 产品定位 | Anthropic 的 agentic coding tool，主要面向本地开发、终端、IDE、桌面端和网页里的代码任务 | OpenAI 的 coding agent，可在云端、CLI、IDE、Web、移动端、CI/CD 等入口工作 |
...
| 更适合 | 个人开发者、本地重度用户、希望细调权限和工作流的人 | 团队协作、并行任务、PR 草稿、代码审查、云端异步工程任务 |

## 1. 产品定位：本地开发搭档 vs 云端任务代理

Claude Code 官方定义是一个能读取代码库、编辑文件、运行命令并接入开发工具的 agentic coding tool。它最核心的体验是从终端、IDE、桌面端或 Web 进入当前项目，让 Claude 直接围绕你的工作目录推进任务。官方文档强调它可以根据自然语言描述实现功能、修 bug、理解代码库。
...
- Claude Code 的手感更像“我在项目里打开一个会写代码的开发搭档”。
- Codex 的手感更像“我把一个工程任务派给云端，让它跑完后带结果回来”。

## 2. 开发流程：谁更适合日常写代码

Claude Code 适合那种连续、贴身、来回迭代的开发节奏。比如你在本地跑测试，看到错误，直接让它分析；它改完文件，你马上复查；再让它继续补边界测试。它的优势是上下文离你的工作目录很近，交互更直接。
...
| testedBy | 编辑实测 |
| evalSet | 同一 Next.js 项目中，完成代码理解任务、`/compare/[slug]` 入口风险扫描、Codex 云端 Markdown-only 验证；确认订阅额度消耗与 API 计费是否分离 |
| sampleSize | 3 个任务；代码理解和入口风险扫描为两个工具各 1 次，Codex 云端验证 1 次 |
| reproducible | true |
| repoUrl | 待补充 |
...
任务 3：Codex 云端检查 `docs/comparison-drafts/claude-code-vs-codex.md`，运行 `npm run lint` / `npm run build`，只允许修改 Markdown，尝试生成 PR 草稿。
...
| `npm run lint` | 已完成 |
| `npm run build` | 已完成 |
| 修改范围 | 只改 Markdown，未改业务代码 |
| 远端分支 | `codex/claude-codex-draft-review` |
| PR 草稿 | 插件创建 draft PR 返回 `403 Resource not accessible by integration`，未能自动创建 |
| 耗时 | 8 分 10 秒 |
| 误判 | 未记录明显误判 |

这次样本覆盖“代码理解/流程解释”“入口风险扫描/建议”和“Codex 云端验证”三类任务，不足以代表真实改代码、测试补全和 code review 等场景。正式发布前，建议至少再补 1 个实际改代码任务或 code review 任务。
...
- [ ] Claude Code：补充桌面端应用版本信息，读取项目、运行 lint/build、提交 diff。
- [x] Codex：连接 GitHub 仓库、云端环境配置、运行 lint/build、Markdown-only 修正、推送分支；PR 草稿因 GitHub integration 403 未自动创建。
