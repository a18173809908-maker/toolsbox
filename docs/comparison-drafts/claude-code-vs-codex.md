...
本次小样本实测中，使用同一任务“分析这个项目的 `/compare/[slug]` 对比页渲染流程”：**Claude Code（VS Code 中使用 Claude Sonnet 4.6）解释更细、更快；Codex（GPT-5.5）解释更完整、速度慢一些，但更会结合项目整体上下文。**
...
| 典型入口 | 终端 `claude`、VS Code、JetBrains、Desktop、Web；本次实测使用 VS Code | `chatgpt.com/codex`、Codex CLI、IDE extension、GitHub `@codex`、SDK |
...
Claude Code 可以通过终端、VS Code、JetBrains、Desktop 和 Web 使用。本次实测走的是 VS Code 中的 Claude Code，而不是 CLI；如果你走终端路径，通常是安装 CLI，进入项目目录，运行 `claude`。
...
| 工具 | 观察结果 |
|---|---|
| Claude Code | VS Code 中使用 Claude Sonnet 4.6；解释更细，响应更快；适合边看代码边追具体实现路径 |
...
- [ ] Claude Code：补充 VS Code 插件/扩展版本，读取项目、运行 lint/build、提交 diff。
