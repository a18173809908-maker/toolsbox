...
## 📋 K 系列任务：Sprint 2 收尾 + 首页打磨（2026-05-09 起）

> **背景**：Claude 已完成首页布局重构（编辑推荐 3 列网格、开发者趋势移到下方、Hot search/decision links 死链修复、admin featured toggle 等，最近 commit 范围 `5e4b997..54d181a`）。本批任务是 Sprint 2 的收尾工程项 + Claude 顺手发现的待修复点。
>
> **Codex 实施顺序建议**：K1 → K2 → K3。每个独立 commit。

---

### K1（最高优先级）：替代品专题 + 对比页 SEO schema

> 对应 sprint-2.md 的 **I8** 任务，详见原任务定义。本节只补充实施细节。

**目标**：
1. 新增 `/alternatives/[slug]` 替代品专题页（首批 5 个）
2. 给 `/compare/[slug]` 加 JSON-LD（FAQPage + BreadcrumbList）
3. sitemap.ts 收录新增路径

**为什么优先级最高**：
- 纯工程，不依赖人工内容
- 顺手修一个死链：首页热搜「Cursor vs Trae」我之前作为「Cursor 替代品」的临时替代跳到了对比页，K1 完成后 `/alternatives/cursor` 才是正经落点（参见 `components/V2Pro.tsx` 第 220 行 hotQueries 数组）
- 放大已发布 10 篇对比页的 SEO 价值

#### K1-A：替代品专题页

**新建文件**：
- `app/alternatives/page.tsx` — 列表页，显示已上线的替代品专题入口
- `app/alternatives/[slug]/page.tsx` — 详情页

**首批 5 个专题**（slug = 被替代的工具 id）：

| slug | 标题 | 替代列表（按 cnAlternatives 字段或手工指定） |
|---|---|---|
...
| `notion-ai` | Notion AI 的国产替代方案 | doubao, kimi, wenxin-yiyan（带文档分析能力的）|
| `runway` | Runway 的国产替代方案 | kling-ai, hailuo-ai, jimeng-ai |

**数据来源**：
1. 优先从被替代工具的 `cnAlternatives` 字段读取（已有数据）
2. 如果该字段为空，从 `data/alternatives.ts` 静态映射兜底（新建）
3. 详情页要展示每个替代工具的：图标、名称、定价、国内访问、为什么是替代品（一句话）

**详情页结构（参考 sprint-2.md I8 + ai-bot.cn 风格）**：
```
Hero
  └ 标题：「<原工具> 的国产替代方案」
  └ 副标题：「海外工具被墙 / 价格贵 / 中文支持差？这些工具能让国内用户直接上手」
被替代工具卡（顶部小卡，链接到 /tools/<id>）
为什么需要替代（300-500 字段落，按需手写或 AI 起草）
替代工具列表（每个 1 张大卡：logo / 名 / 定价 / 国内访问 / 一句话理由 / 跳到详情页按钮）
  └ 至少 3 个，按推荐度排序
对比页推荐（如果有相关 comparison：「Cursor vs Trae」对比页直接挂在替代列表下方）
JSON-LD ItemList schema
```

**SEO metadata**（每个专题独立）：
- title: `Cursor 的国产替代方案 — 5 个国内可用的 AI 编程工具 | AIBoxPro`
- description: `Cursor 在国内访问不稳定？盘点 5 个国产替代工具：Trae、Claude Code…，覆盖 IDE、终端、补全各场景。`
- canonical: `/alternatives/cursor`

#### K1-B：对比页 JSON-LD schema

**修改 `app/compare/[slug]/page.tsx`**：

1. **FAQPage schema**：从 `comparisons.body` markdown 提取 `## 标题` 后的段落作为 Q&A，或用 `comparisons.faqs` 字段（如果有）
...
3. **Article schema**（如果还没有）：包含 datePublished / dateModified / author / publisher

#### K1-C：sitemap 收录

修改 `app/sitemap.ts`：
- 加入 `/alternatives` 列表页
- 加入所有已发布的 `/alternatives/[slug]`（按 K1-A 的 5 个 slug 静态列出，或动态从 DB 派生）
...
- 首页热搜「Cursor 替代品」临时改回（如果有这个标签的话），现在指向 `/alternatives/cursor`

---

### K2（中优先级）：搜索框 placeholder 引导优化

**问题**：首页搜索框 placeholder 是 `输入工具名或对比，例如 Claude Code vs Codex`，用户容易输入「AI 写论文」「免费 AI 工具」等自然语言短语，但 `/tools?q=xxx` 是按 name/zh/en 做 ILIKE，搜不到任何工具。

**修复**：
1. `components/V2Pro.tsx` 第 340 行 placeholder 改为：
   ```
   输入工具名（如 Cursor / Claude / 豆包）
   ```
2. 搜索框下方的 5 个热搜标签保留为现状（已经是有效跳转，作为「自然语言查询」的兜底）

**注意**：长期方案是接 NLU 把短语映射到路由（「AI 写论文」→ `/scenes/ai-write-thesis`），但工程量大，本次只改 placeholder。

---

### K3（低优先级）：admin 工具详情页加 featured toggle

**问题**：`/admin/featured` 列表页可以批量切换 featured，但审核工作流是「在 `/admin/tools/[id]` 看完候选 → approve → 跳回列表」。如果想给一个新 approve 的工具立刻设置 featured，得多一步导航。

**修复**：在 `/admin/tools/[id]/page.tsx` 中（仅 candidate.status === 'approved' 且对应 tools 行存在时）加一个 FeaturedToggle 入口。

**实现要点**：
1. 复用 `app/admin/featured/FeaturedToggle.tsx` 组件
2. 在详情页底部加一个 section「编辑推荐设置」
3. 加载时读 `tools.featured`（用 candidate.slug 作为 tool.id 查询）
4. 如果该候选还没 approve（slug 在 tools 表不存在），显示「批准后才能设置」灰态

#### 验证标准

- `/admin/tools/[id]` 已 approve 的候选页面底部有 FeaturedToggle
- 切换后首页 `/` 立即生效（revalidatePath 已在 K3 复用的 API 里处理）

---

### K 系列以外

以下任务在 sprint-2.md 中已详细定义，**Codex 不要在这里重复**，直接按 sprint-2.md 实施：

| 任务 | 类型 | 阻塞 |
|---|---|---|
| **I7** 首份 Lab 报告 | 编辑实测 | 必须人工测试 Claude Code vs Cursor 配对 |
| **I9** 连通性数据 | 30 条编辑实测录入 | 需要人 / 三网卡环境 |
| **I10** 图文自动生成 | 工程（puppeteer）| 无人工阻塞，但工程量 3-5 天 |
| **I11** 社区分发 SOP | 运营文档 | 无 |
| **I12** 工具方互推 | 运营文档 | 无 |
| **I13** 小红书账号 | 人工注册 | 需要手机号 |

**建议 Codex 在做完 K1-K3 后**，按以下顺序推进：
1. **I11 + I12** 一次性把两个运营 SOP 文档写完（30 分钟 ×2，纯文档）
2. **I10** 图文自动生成（3-5 天，复杂工程）
3. I7/I9/I13 等用户参与后再继续
