...
const C = {
  ...
  ink: '#1F2937', rule: '#E8D5B7', bg: '#FFF7ED', panel: '#FFFFFF',
};

type Props = {
  id: string;
  apiBase: string;   // e.g. /api/admin/verdicts
  listPath: string;  // e.g. /admin/verdicts
  publishLabel?: string;
};

export default function DraftReviewActions({ id, apiBase, listPath, publishLabel = '发布' }: Props) {
  ...
  async function post(action: 'publish' | 'reject') {
    setLoading(true);
    setError(null);
    try {
    ...
    <section style={{ background: C.panel, border: `1px solid ${C.rule}`, borderRadius: 12, padding: 24, marginTop: 16 }}>
      ...
      {!rejecting ? (
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button disabled={loading} onClick={() => post('publish')} style={btn(C.greenBg, C.green, loading)}>{publishLabel}</button>
        ...
        <div>
          <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3}
            placeholder="拒绝原因（可选）"
            style={{ width: '100%', boxSizing: 'border-box', padding: 10, border: `1px solid ${C.rule}`, borderRadius: 8, background: C.bg, marginBottom: 12 }} />
...
function btn(bg: string, color: string, disabled: boolean): React.CSSProperties {
  return { padding: '9px 16px', border: 'none', borderRadius: 8, background: disabled ? '#E5E7EB' : bg, color: disabled ? '#9CA3AF' : color, cursor: disabled ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: 13 };
}
